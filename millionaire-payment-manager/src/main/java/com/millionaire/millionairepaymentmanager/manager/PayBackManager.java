...
//          到期日期
            ...
            LocalDateTime startDateTime = LocalDateTime.parse(startDate, timeFormatter);
            LocalDateTime endDateTime = LocalDateTime.parse(endDate, timeFormatter);

            int startYear = startDateTime.getYear();
            int startMonth = startDateTime.getMonth().getValue();

            int endMonth = endDateTime.getMonth().getValue();
            int endDay = endDateTime.getDayOfMonth();

//        分期付款次数
            int taskTimes = (endDay - startYear) * 12 + (endMonth - startMonth);
...
//                       计算两个日期之间的天数
                        Period periodToNext = Period.between(nextMonths, fixedDate);
...
//            付款金额，以分为单位
                int backAmount = investmentUser.getInvestmentAmount() + expectIncome;
                timerTaskInvestment.setPaybackAmount(backAmount);
//            表示本息一次付清
                timerTaskInvestment.setExecuteType((byte) 30);
...
//        SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");//可以方便地修改日期格式
        ...
        String fixedTime = startYear+"-"+String.format("%02d", startMonth)+"-20"+" 00:00:00";
        DateTimeFormatter timeFormatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");
        LocalDateTime nowDateTime = LocalDateTime.parse(fixedTime, timeFormatter);
        LocalDateTime nextYear = nowDateTime.plus(1, ChronoUnit.MONTHS);

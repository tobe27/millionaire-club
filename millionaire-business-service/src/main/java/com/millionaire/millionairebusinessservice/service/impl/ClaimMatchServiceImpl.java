...
public class ClaimMatchServiceImpl implements ClaimMatchService {
    ...
    public List<InvestmentUser> listRecommendInvestmentUser(long claimID) {
        ...
        for (InvestmentUser investmentUser : usableInvestmentUserList) {
            logger.info("初始可用用户投资id:{}", investmentUser.getId());
        }
        logger.info("-------初始可用用户投资表-----------");
        //同一用户同一时间多笔投资不能匹配相同债权
        //根据债权id查询用户投资列表
        List<InvestmentUser> matchInvestUserList = investmentUserMapper.selectMatchedInvestmentUser(claimID);
        logger.info("======================已经匹配该债权的用户id======================================");
        for (InvestmentUser user : matchInvestUserList) {
            logger.info("已经匹配的该债权的用户id:{}", user.getUid());
        }
        logger.info("======================已经匹配该债权的用户id======================================");
//        List<Long> uidList = investmentUserMapper.selectMatchedUID(claimID);
        ...
        while (investmentUserIterator.hasNext()) {
            ...
            for (InvestmentUser user : matchInvestUserList) {
                ...
                if (user.getUid() == investmentUser.getUid()) {
        ...
        logger.info("=====================剔除重复id后的用户投资================================");
        logger.info("剔除重复id后可用用户投资数:{}", usableInvestmentUserList.size());
        for (InvestmentUser user : usableInvestmentUserList) {
            logger.info("剔除重复id后可用用户投资id:{}", user.getId());
        }
        logger.info("=====================剔除重复id后的用户投资================================");
        ...
        while (investmentUserIterator2.hasNext()) {
            ...
            if (investmentUser.getInvestmentAmount() > claimInfo.getUnMatchAmount()) {
                ...
                logger.info("用户投资id:{},投资金额:{} ", investmentUser.getId(), investmentUser.getInvestmentAmount());
                logger.info("债权未匹配金额:{}", claimInfo.getUnMatchAmount());
                logger.info("---------剔除过大用户投资---------");
                investmentUserIterator2.remove();
            }
        }
        logger.info("=====================剔除过大用户投资后的用户投资================================");
        logger.info("剔除过大用户投资后可用用户投资数：{}", usableInvestmentUserList.size());
        for (InvestmentUser user : usableInvestmentUserList) {
            logger.info("剔除过大用户投资可用用户投资id:{}", user.getId());
        }
        logger.info("=====================剔除过大用户投资后的用户投资================================");
        logger.info("=====================最后可用的用户投资================================");
        logger.info("最后可用用户投资数:{}", usableInvestmentUserList.size());
        for (InvestmentUser investmentUser : usableInvestmentUserList) {
            logger.info("剔除后可用用户投资id:{}", investmentUser.getId());
        }
        logger.info("=====================最后可用的用户投资================================");
        //设置时间格式用于后续对比
        ...
        for (InvestmentUser investmentUser : usableInvestmentUserList) {
            logger.info("根据金额时间排序后可用用户投资id:{}", investmentUser.getId());
        }
        logger.info("-------根据金额时间排序后可用用户投资表-----------");
